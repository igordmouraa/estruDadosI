class FormaGeometrica {
  ...
  set base(valor) {
    if (typeof valor !== "number" || valor <= 0) {
      throw new Error("A base precisa ser numérica e maior que zero");
    }
    this.#base = valor;
  }
  ...
  set altura(valor) {
    if (typeof valor !== "number" || valor <= 0) {
      throw new Error("A altura precisa ser numérica e maior que zero");
    }
    this.#altura = valor;
  }
  ...
  set tipo(valor) {
    if (! ["R", "T", "E"].includes(valor.toUpperCase())) {
      throw new Error("O tipo precisa ser um dos valores: R, T ou E");
    }
...
console.log(`Área de um retângulo de 7x12: ${forma1.calcArea()}`);
